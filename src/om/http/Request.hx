...
class Request {
	...
	public static function read( buf : Bytes, pos : Int, len : Int ) : Request {

		var i = new BytesInput( buf, pos, len );
		var line = i.readLine();
		var rexp = ~/(GET|POST) \/(.*) HTTP\/(1\.1)/;
		if( !rexp.match( line ) ) {
			return throw new Error( StatusCode.BAD_REQUEST );
		}

		var url = rexp.matched(2); //TODO check url, security!
		var version = rexp.matched(3);

		var r = new Request( url, version );
		var _method : Method = rexp.matched(1);
		/*
